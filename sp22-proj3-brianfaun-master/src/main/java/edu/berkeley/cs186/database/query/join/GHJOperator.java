...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        for (Record record : records) {
            ...
            hashMod = hash % partitions.length;
            if (hashMod < 0) {
                hashMod += partitions.length;
            }
            partitions[hashMod].add(record);
        }
    }
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        Map<DataBox, List<Record>> H = new HashMap<>();
        for (Record build : buildRecords) {
            DataBox buildColumn = build.getValue(buildColumnIndex);
            if (!H.containsKey(buildColumn)) {
                H.put(buildColumn, new ArrayList<>());
            }
            H.get(buildColumn).add(build);
        }
        for (Record probe : probeRecords) {
            DataBox probeColumn = probe.getValue(probeColumnIndex);
            if (!H.containsKey(probeColumn)) {
                continue;
            }
            for (Record buildRecord : H.get(probeColumn)) {
                Record grace; 
                if (probeFirst) {
                    grace = probe.concat(buildRecord);
                } else {
                    grace = buildRecord.concat(probe);
                }
