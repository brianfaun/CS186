...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException{
        ...
        Optional<Pair<DataBox, Long>> insertPair = root.put(key, rid);
        if (insertPair.isPresent()) {
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();
            keys.add(insertPair.get().getFirst());
            children.add(root.getPage().getPageNum());
            children.add(insertPair.get().getSecond());
            InnerNode insertRoot = new InnerNode(metadata, bufferManager, keys, children, lockContext);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId>{
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (nextIter.hasNext() || nextNode.getRightSibling().isPresent()) {
                return true;
            } else {
                return false;
            }
        ...
        public RecordId next() {
            // TODO(proj2): implement
            if (!hasNext()) { //In the same leaf node
