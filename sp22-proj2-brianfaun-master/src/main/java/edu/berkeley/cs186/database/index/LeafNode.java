...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) throws BPlusTreeException{
        ...
        LeafNode newNode = new LeafNode(metadata, bufferManager, rightKeys, rightRecords, rightSibling, treeContext);
        long pageNum = newNode.getPage().getPageNum();

        //Overflow - Left Node
        this.keys = leftKeys;
        this.rids = leftRecords;
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext() && keys.size() < Math.ceil(fillFactor * 2 * d)){
            Pair<DataBox, RecordId> rec = data.next();
            DataBox key = rec.getFirst();
            RecordId rId = rec.getSecond();
        ...
        if (data.hasNext()){
            ...
            int i2 = InnerNode.numLessThanEqual(key, keys);
            keys.add(i2, key);
            rids.add(i2, rId);

            int maxSpace = (int) Math.ceil(fillFactor * 2 * d);
            List<DataBox> leftK = keys.subList(0, maxSpace);
            List<DataBox> rightK = keys.subList(maxSpace, maxSpace + 1);

            List<RecordId> leftR = rids.subList(0, maxSpace);
            List<RecordId> rightR = rids.subList(maxSpace, maxSpace + 1);

            keys = leftK;
            rids = leftR;

            LeafNode rightLeaf = new LeafNode(metadata, bufferManager, rightK, rightR, rightSibling, treeContext);
            Long curPageNum = rightLeaf.getPage().getPageNum();
            rightSibling = Optional.of(curPageNum);
            sync();

            Pair pushingPair = new Pair<>(rightK.get(0), curPageNum);
    ...
    public void remove(DataBox key) { 
        ...
        if (index != -1) {
            keys.remove(index);
            rids.remove(index); //sync can be outside if 
        }
        sync();
        return;
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
       ...
       Page page = bufferManager.fetchPage(treeContext, pageNum);
        ...
        LeafNode newLeaf = new LeafNode(metadata, bufferManager, page, keys,
